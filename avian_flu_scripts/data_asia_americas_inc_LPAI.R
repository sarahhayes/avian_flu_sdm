## 20/003/2024
## Exploring data availability across Asia and the Americas, but including LPAI to see how many points it adds.  
...
## if we only want HPAI 
poss_hpai <- bv_americas_asia %>% filter(str_detect(Subtype,"H5") | str_detect(Subtype, "H7"))

# There are 736 observations for HPAI in bvbrc and 21237 that includes all subtypes. 

subtypes_bvbrc <- as.data.frame(table(bv_americas_asia$Subtype))

#write.csv(subtypes_bvbrc, "data/flu_data/prepped_data/asia_americas_bvbrc_inc_lpai.csv")
...
bv_americas_asia <- rename(bv_americas_asia, Latitude = Collection.Latitude,
  ...
  bv_americas_asia <- dplyr::select(bv_americas_asia, c(Latitude, Longitude, observation.date, Species,
                                        Country, Serotype, source, Region))

americas_asia_data <- rbind(fao_data_trim_asia_americas, inf_a_hpai_americas_asia, bv_americas_asia)
...
## keeping all subtypes at this stage
library(sf)
...
#png("plots/americas_pos_data_inc_LPAI.png", width = 480, height = 480)
par(mar = c(0,0,0,0))
plot(americas_map)
...
#png("plots/asia_pos_data_inc_LPAI.png", width = 480, height = 480)
par(mar = c(0,0,0,0))
plot(asia_map)
...
#Start on 5th May 1986 as that's a Monday
start.plot_americas <- c("1986-01-05", "2023-12-31")
...
all_dates_americas_data$year_of_study <- all_dates_americas_data$year - 1986
...
all_dates_americas_data_nodups$year_of_study <- all_dates_americas_data_nodups$year - 1986
...
yearlabs_americas <- c("1986", "1987", "1988", "1989", "1990", "1991", "1992", "1993", "1994", "1995",
...
#ggsave("plots/time_series_americas_with_LPAI.png")


# One downside of this plot is that it doesn't have duplicates removed. 
ggplot(data = all_dates_americas_data_nodups, 
       aes(x = week_seq, y = n)) +
  geom_line(lwd = 0.5, col = "pink") + 
  scale_color_brewer(palette="Paired", na.translate = F) + 
  theme(axis.text.x = element_text(angle=90, margin = margin(t = .2, unit = "cm"), 
                                   face = "bold", size = 12, vjust = 0.5),
        axis.text.y = element_text(face = "bold", size = 12),
  ...
  labs(y = "Number of weekly cases", x = "Year", size = 18, title = "Americas") +
  scale_x_continuous(breaks = weekbreaks_americas, labels = yearlabs_americas)
ggsave("plots/time_series_americas_lpai_dups_removed.png")
