...
AVONET_df <- read_excel("data_offline/eco_phylo_data/AVONETSupplementarydataset1.xlsx",
...
EltonTraits_df <- EltonTraits_df %>% filter(!is.na(SpecID))
...
# Load data
CLOVER_df <- read.csv(
  "data_offline/eco_phylo_data/CLOVER_1.0_Viruses_AssociationsFlatFile.csv")
...
IUCN_df <- rbind(
  read.csv("data_offline/eco_phylo_data/iucn-data-vol1/all_other_fields.csv"),
  read.csv("data_offline/eco_phylo_data/iucn-data-vol2/all_other_fields.csv"))
...
IUCN_by_species <- IUCN_df[map_dbl(position_in_IUCN, first), ]
...
{
  if (!file.exists("data_offline/eco_phylo_data/mean_phylo_distances.rds")){
    # Attempt to load in BirdTree data
    bird_tree <- read.tree("data_offline/eco_phylo_data/BirdzillaHackett10.tre")
    ...
    saveRDS(dmat_mean, "data_offline/eco_phylo_data/mean_phylo_distances.rds")
  }
  else{
    dmat_mean <- readRDS("data_offline/eco_phylo_data/mean_phylo_distances.rds")
  }
}
...
basic_model <- bart(x.train = xtrain,
                    y.train = ytrain,
                    x.test = xtest,
                    keeptrees = TRUE)
...
## Use saturated model instead
# varimp_raw <- basic_model$varcount %>%
#   as.data.frame %>%
#   mutate(./rowSums(.)) %>%
#   mutate(./max(.))
...
varimp_summ <- varimp_summ %>% mutate(var = factor(var, levels = ordered_var), upper = mean + sd, lower = mean - sd)
...
for(i in 1:length(vars)){
  ...
  if (length(fullvarname) == 1) {
    ...
    if (all(varselect$fit$data@x[, fullvarname] %in% 0:1)){
...
p <- wrap_plots(panel_list, ncol = 3) +
  plot_layout(guides = "collect", axis_titles = "collect") +
  plot_annotation(tag_levels = 'A')

ggsave(paste0("plots\\prelim_sppmodel_",region,".png"), plot = p, width = 15, height = 10)
