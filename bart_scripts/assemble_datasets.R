...
covstack <- raster::stack(paste(PATH_TO_DATA, "AI_S2_SDM_storage/quarterly_covariates_eco_seasons/all_q_covs_10k.tif", sep = ""))
...
write.csv(training_data, "training_sets/training_data_eco_seasons_A_Q1.csv", row.names = FALSE)
...
write.csv(test_data, "training_sets/test_data_eco_seasons_A_Q1.csv", row.names = FALSE)
...
training_data$y <- training_coords$pos
training_data$ri <- sapply(ri_train$country, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(training_data, "training_sets/training_data_eco_seasons_A_Q2.csv", row.names = FALSE)
...
test_data$y <- test_coords$pos
test_data$ri <- sapply(ri_test, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(test_data, "training_sets/test_data_eco_seasons_A_Q2.csv", row.names = FALSE)
...
if (!SKIP_AQ3){
  ...
  training_data$y <- training_coords$pos
  training_data$ri <- sapply(ri_train$country, FUN=function(i){country_lookup$country[country_lookup$val==i]})
  write.csv(training_data, "training_sets/training_data_eco_seasons_A_Q3.csv", row.names = FALSE)
  ...
  test_data$y <- test_coords$pos
  test_data$ri <- sapply(ri_test, FUN=function(i){country_lookup$country[country_lookup$val==i]})
  write.csv(test_data, "training_sets/test_data_eco_seasons_A_Q3.csv", row.names = FALSE)
}
################################################################################
# Dataset A Q4
...
training_data$y <- training_coords$pos
training_data$ri <- sapply(ri_train$country, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(training_data, "training_sets/training_data_eco_seasons_A_Q4.csv", row.names = FALSE)
...
test_data$y <- test_coords$pos
test_data$ri <- sapply(ri_test, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(test_data, "training_sets/test_data_eco_seasons_A_Q4.csv", row.names = FALSE)
...
training_data$y <- training_coords$pos
training_data$ri <- sapply(ri_train$country, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(training_data, "training_sets/training_data_eco_seasons_B_Q1.csv", row.names = FALSE)

#### Now do testing ####
# Load testing data
test_coords <- readRDS("training_sets/test_coords_B_Q1.RDS")
# # Identify and plot samples where a country could not be assigned - should find
# # it's all on-water samples.
# 
ri_test <- data.frame(raster::extract(country_rast, test_coords[, 1:2]))
...
if (PLOT_PROBLEM_SAMPLES){
  plot(euro_map_crop,
       ...
       axes = FALSE,
       buffer = FALSE,
       main = "Points with missing country data, B Q1 test")
  plot(mismatch_pts, add = T, col = "red", pch = 16, cex = 1.)
}
# Try replacing NA pixels with nearest non-NA
country_shift <- ri_test
...
if (length(bad_rows)>0){
  ...
  if (PLOT_PROBLEM_SAMPLES){
    plot(euro_map_crop,
         ...
         axes = FALSE,
         buffer = FALSE,
         main = "Points missing country data after imputation, B Q1 test")
    plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
  }
  cat("Removing ", length(bad_rows), " datapoints with NA covariates.\n")
  country_shift <- country_shift[-bad_rows, ]
...
if (PLOT_COUNTRY_VALIDATION){
  ...
  plot(euro_map_crop,
       ...
       buffer = FALSE,
       xmin = euro_ext@xmin,
       main = "Visual inspection of country assignments, B Q1 test")
  pts_to_plot <- sample(1:nrow(test_coords), 25)
  for (i in 1:25){
    pts_pos <- terra::vect(test_coords[pts_to_plot[i], ], geom=c("X", "Y"),
                           crs =  crs)
  ...
  plot(euro_map_crop,
       ...
       buffer = FALSE,
       xmin = euro_ext@xmin,
       main = "Country assignments obtained through imputation, B Q1 test")
  pts_to_plot <- sample(1:nrow(test_coords), 25)
  for (i in mismatch_samples){
    pts_pos <- terra::vect(test_coords[i, ], geom=c("X", "Y"),
                           crs =  crs)
...
if (length(bad_rows)>0){
  ...
  if (PLOT_PROBLEM_SAMPLES){
    plot(euro_map_crop,
         ...
         axes = FALSE,
         buffer = FALSE,
         main = "Points with missing covariates, B Q1 test")
    plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
  }
  ...
  if (length(bad_rows)>0){
    ...
    if (PLOT_PROBLEM_SAMPLES){
      plot(euro_map_crop,
           ...
           axes = FALSE,
           buffer = FALSE,
           main = "Points with missing covariates after imputation, B Q1 test")
      plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
    }
...
test_data$y <- test_coords$pos
test_data$ri <- sapply(ri_test, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(test_data, "training_sets/test_data_eco_seasons_B_Q1.csv", row.names = FALSE)
...
training_data$y <- training_coords$pos
training_data$ri <- sapply(ri_train$country, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(training_data, "training_sets/training_data_eco_seasons_B_Q2.csv", row.names = FALSE)

#### Now do testing ####
# Load testing data
test_coords <- readRDS("training_sets/test_coords_B_Q2.RDS")
# # Identify and plot samples where a country could not be assigned - should find
# # it's all on-water samples.
# 
ri_test <- data.frame(raster::extract(country_rast, test_coords[, 1:2]))
...
if (PLOT_PROBLEM_SAMPLES){
  plot(euro_map_crop,
       ...
       axes = FALSE,
       buffer = FALSE,
       main = "Points with missing country data, B Q2 test")
  plot(mismatch_pts, add = T, col = "red", pch = 16, cex = 1.)
}
# Try replacing NA pixels with nearest non-NA
country_shift <- ri_test
...
if (length(bad_rows)>0){
  ...
  if (PLOT_PROBLEM_SAMPLES){
    plot(euro_map_crop,
         ...
         axes = FALSE,
         buffer = FALSE,
         main = "Points missing country data after imputation, B Q2 test")
    plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
  }
  cat("Removing ", length(bad_rows), " datapoints with NA covariates.\n")
  country_shift <- country_shift[-bad_rows, ]
...
if (PLOT_COUNTRY_VALIDATION){
  ...
  plot(euro_map_crop,
       ...
       buffer = FALSE,
       xmin = euro_ext@xmin,
       main = "Visual inspection of country assignments, B Q2 test")
  pts_to_plot <- sample(1:nrow(test_coords), 25)
  for (i in 1:25){
    pts_pos <- terra::vect(test_coords[pts_to_plot[i], ], geom=c("X", "Y"),
                           crs =  crs)
  ...
  plot(euro_map_crop,
       ...
       buffer = FALSE,
       xmin = euro_ext@xmin,
       main = "Country assignments obtained through imputation, B Q2 test")
  pts_to_plot <- sample(1:nrow(test_coords), 25)
  for (i in mismatch_samples){
    pts_pos <- terra::vect(test_coords[i, ], geom=c("X", "Y"),
                           crs =  crs)
...
if (length(bad_rows)>0){
  ...
  if (PLOT_PROBLEM_SAMPLES){
    plot(euro_map_crop,
         ...
         axes = FALSE,
         buffer = FALSE,
         main = "Points with missing covariates, B Q2 test")
    plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
  }
  ...
  if (length(bad_rows)>0){
    ...
    if (PLOT_PROBLEM_SAMPLES){
      plot(euro_map_crop,
           ...
           axes = FALSE,
           buffer = FALSE,
           main = "Points with missing covariates after imputation, B Q2 test")
      plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
    }
...
test_data$y <- test_coords$pos
test_data$ri <- sapply(ri_test, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(test_data, "training_sets/test_data_eco_seasons_B_Q2.csv", row.names = FALSE)
...
training_data$y <- training_coords$pos
training_data$ri <- sapply(ri_train$country, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(training_data, "training_sets/training_data_eco_seasons_B_Q3.csv", row.names = FALSE)

#### Now do testing ####
# Load testing data
test_coords <- readRDS("training_sets/test_coords_B_Q3.RDS")
# # Identify and plot samples where a country could not be assigned - should find
# # it's all on-water samples.
# 
ri_test <- data.frame(raster::extract(country_rast, test_coords[, 1:2]))
...
if (PLOT_PROBLEM_SAMPLES){
  plot(euro_map_crop,
       ...
       axes = FALSE,
       buffer = FALSE,
       main = "Points with missing country data, B Q3 test")
  plot(mismatch_pts, add = T, col = "red", pch = 16, cex = 1.)
}
# Try replacing NA pixels with nearest non-NA
country_shift <- ri_test
...
if (length(bad_rows)>0){
  ...
  if (PLOT_PROBLEM_SAMPLES){
    plot(euro_map_crop,
         ...
         axes = FALSE,
         buffer = FALSE,
         main = "Points missing country data after imputation, B Q3 test")
    plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
  }
  cat("Removing ", length(bad_rows), " datapoints with NA covariates.\n")
  country_shift <- country_shift[-bad_rows, ]
...
if (PLOT_COUNTRY_VALIDATION){
  ...
  plot(euro_map_crop,
       ...
       buffer = FALSE,
       xmin = euro_ext@xmin,
       main = "Visual inspection of country assignments, B Q3 test")
  pts_to_plot <- sample(1:nrow(test_coords), 25)
  for (i in 1:25){
    pts_pos <- terra::vect(test_coords[pts_to_plot[i], ], geom=c("X", "Y"),
                           crs =  crs)
  ...
  plot(euro_map_crop,
       ...
       buffer = FALSE,
       xmin = euro_ext@xmin,
       main = "Country assignments obtained through imputation, B Q3 test")
  pts_to_plot <- sample(1:nrow(test_coords), 25)
  for (i in mismatch_samples){
    pts_pos <- terra::vect(test_coords[i, ], geom=c("X", "Y"),
                           crs =  crs)
...
if (length(bad_rows)>0){
  ...
  if (PLOT_PROBLEM_SAMPLES){
    plot(euro_map_crop,
         ...
         axes = FALSE,
         buffer = FALSE,
         main = "Points with missing covariates, B Q3 test")
    plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
  }
  ...
  if (length(bad_rows)>0){
    ...
    if (PLOT_PROBLEM_SAMPLES){
      plot(euro_map_crop,
           ...
           axes = FALSE,
           buffer = FALSE,
           main = "Points with missing covariates after imputation, B Q3 test")
      plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
    }
...
test_data$y <- test_coords$pos
test_data$ri <- sapply(ri_test, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(test_data, "training_sets/test_data_eco_seasons_B_Q3.csv", row.names = FALSE)
...
training_data$y <- training_coords$pos
training_data$ri <- sapply(ri_train$country, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(training_data, "training_sets/training_data_eco_seasons_B_Q4.csv", row.names = FALSE)

#### Now do testing ####
# Load testing data
test_coords <- readRDS("training_sets/test_coords_B_Q4.RDS")
# # Identify and plot samples where a country could not be assigned - should find
# # it's all on-water samples.
# 
ri_test <- data.frame(raster::extract(country_rast, test_coords[, 1:2]))
...
if (PLOT_PROBLEM_SAMPLES){
  plot(euro_map_crop,
       ...
       axes = FALSE,
       buffer = FALSE,
       main = "Points with missing country data, B Q4 test")
  plot(mismatch_pts, add = T, col = "red", pch = 16, cex = 1.)
}
# Try replacing NA pixels with nearest non-NA
country_shift <- ri_test
...
if (length(bad_rows)>0){
  ...
  if (PLOT_PROBLEM_SAMPLES){
    plot(euro_map_crop,
         ...
         axes = FALSE,
         buffer = FALSE,
         main = "Points missing country data after imputation, B Q4 test")
    plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
  }
  cat("Removing ", length(bad_rows), " datapoints with NA covariates.\n")
  country_shift <- country_shift[-bad_rows, ]
...
if (PLOT_COUNTRY_VALIDATION){
  ...
  plot(euro_map_crop,
       ...
       buffer = FALSE,
       xmin = euro_ext@xmin,
       main = "Visual inspection of country assignments, B Q4 test")
  pts_to_plot <- sample(1:nrow(test_coords), 25)
  for (i in 1:25){
    pts_pos <- terra::vect(test_coords[pts_to_plot[i], ], geom=c("X", "Y"),
                           crs =  crs)
  ...
  plot(euro_map_crop,
       ...
       buffer = FALSE,
       xmin = euro_ext@xmin,
       main = "Country assignments obtained through imputation, B Q4 test")
  pts_to_plot <- sample(1:nrow(test_coords), 25)
  for (i in mismatch_samples){
    pts_pos <- terra::vect(test_coords[i, ], geom=c("X", "Y"),
                           crs =  crs)
...
if (length(bad_rows)>0){
  ...
  if (PLOT_PROBLEM_SAMPLES){
    plot(euro_map_crop,
         ...
         axes = FALSE,
         buffer = FALSE,
         main = "Points with missing covariates, B Q4 test")
    plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
  }
  ...
  if (length(bad_rows)>0){
    ...
    if (PLOT_PROBLEM_SAMPLES){
      plot(euro_map_crop,
           ...
           axes = FALSE,
           buffer = FALSE,
           main = "Points with missing covariates after imputation, B Q4 test")
      plot(bad_pts, add = T, col = "red", pch = 16, cex = 1.)
    }
...
test_data$y <- test_coords$pos
test_data$ri <- sapply(ri_test, FUN=function(i){country_lookup$country[country_lookup$val==i]})
write.csv(test_data, "training_sets/test_data_eco_seasons_B_Q4.csv", row.names = FALSE)
